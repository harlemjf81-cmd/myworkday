...
interface IdealEarningsModalProps {
  currentIdealEarnings: number;
  currencySymbol: string;
  onSave: (newIdealEarnings: number) => void;
  onClose: () => void;
}

export const IdealEarningsModal: React.FC<IdealEarningsModalProps> = ({ currentIdealEarnings, currencySymbol, onSave, onClose }) => {
  const { t } = useTranslation();
  const [newIdealEarnings, setNewIdealEarnings] = useState<string>(currentIdealEarnings.toString());
  const [error, setError] = useState<string>('');

  const handleSave = () => {
    const earningsValue = parseFloat(newIdealEarnings);
    if (isNaN(earningsValue) || earningsValue < 0) {
      setError(t('modals.idealDailyEarnings.error'));
      return;
    }
    setError('');
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    setNewIdealEarnings(e.target.value);
    if (error) setError('');
  };
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 backdrop-blur-sm flex items-center justify-center p-4 z-[100]">
      <div className="bg-white dark:bg-slate-800 p-6 sm:p-8 rounded-xl shadow-2xl w-full max-w-md transform transition-all">
        <h2 className="text-xl sm:text-2xl font-semibold text-slate-800 dark:text-slate-100 mb-6">{t('modals.idealDailyEarnings.title')}</h2>
        
        <div>
          <label htmlFor="idealEarnings" className="block text-sm font-medium text-slate-600 dark:text-slate-300 mb-1">
            {t('modals.idealDailyEarnings.label', { currencySymbol })}
          </label>
          <input
            type="number"
            id="idealEarnings"
            value={newIdealEarnings}
            ...
            placeholder={t('modals.idealDailyEarnings.placeholder')}
            min="0"
            step="1"
            autoFocus
          />
          {error && <p className="text-red-500 text-sm mt-1">{error}</p>}
          <p className="text-xs text-slate-500 dark:text-slate-400 mt-2">{t('modals.idealDailyEarnings.description')}</p>
        </div>
        ...
        <div className="mt-8 flex flex-col sm:flex-row sm:justify-end space-y-3 sm:space-y-0 sm:space-x-3">
          ...
          >
            {t('modals.idealDailyEarnings.saveButton')}
          </button>
        </div>
      </div>
    </div>
...
export default IdealEarningsModal;